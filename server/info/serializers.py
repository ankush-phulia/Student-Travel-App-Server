...
class NotificationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ("id", "user_from", "user_to", "title",
                  "description", "notif_type", "creation_time", "resolved")

    def create(self, validated_data):
        # print(">>>>>>>>>>",validated_data.pop('user_to'))
        user_to = User.objects.get(
            username=validated_data.pop('user_to')["username"])
        user_from = User.objects.get(
            username=validated_data.pop('user_from')["username"])
...
class LocationPointSerializer(serializers.ModelSerializer):
    class Meta:
        model = LocationPoint
        fields = ("id", "location_name", "latitude",
                  "longitude", "location_type", "rating")
