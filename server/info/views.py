...
def match_journeys(user, jrny):
    ...
    for x in Journey.objects.filter(posted=True, closed=False):
        ...
        cps = [
            jp.location.location_name for jp in JourneyPoint.objects.filter(journey=x)]
        print(cps)
...
def match_trips(user, trp):
    ...
    lis = []
    points = [
        tp.location.location_name for tp in TripPoint.objects.filter(trip=trp)]
    for x in Trip.objects.filter(posted=True, closed=False):
        # print(jrny.destination)
        cps = [
            tp.location.location_name for tp in TripPoint.objects.filter(trip=x)]
        # print(cps)
...
class UserInformationUpdate(APIView):
    ...
    def post(self, request, format=None):
        ...
        try:
            ...
            facebook_link = validated_data.get(
                "facebook_link", ui.facebook_link)
...
class JourneyPointsList(APIView):
    ...
    def get(self, request, format=None):
        locs = LocationPoint.objects.filter(
            user=request.user, location_type="Journey Point")
        ...
        try:
            ...
            for i, x in enumerate(checkpoints):
                loc = LocationPoint.objects.get(
                    location_name=x["location"]["location_name"], user=user)
                JourneyPoint.objects.create(
                    location=loc, transport=x["transport"], point_id=x["point_id"], journey=jrny)
            return Response(validated_data, status=status.HTTP_201_CREATED)
        except:
            return Response({}, status=status.HTTP_400_BAD_REQUEST)
...
def Dashboard(request):
    ...
    data["notifs"] = Notification.objects.filter(
        user_to=user, creation_time__gte=last_lg)
    print("Data is ")
    print(last_lg)
    pprint(data)
...
class ModifyInfoForm(forms.Form):
    ...
    gender = forms.ChoiceField(required=False, choices=(
        (None, ''), ('Female', 'Female'), ('Male', 'Male'), ('Other', 'Other')))
...
class NotificationForm(forms.Form):
    ...
    notification_type = forms.ChoiceField(choices=((None, ''), ('Logistics Related', 'Logistics Related'), (
        'Trip Related', 'Trip Related'), ('Journey Related', 'Journey Related')))
    title = forms.CharField(required=True)
    description = forms.CharField(
        widget=forms.Textarea, required=False, initial="")
    layout = Layout(Fieldset("Provide the notification here",
                             'to_user', 'notification_type', "title", "description"))
...
class LocationForm(forms.Form):
    ...
    location_type = forms.ChoiceField(choices=(
        (None, ''), ('Trip Point', 'Trip Point'), ('Journey Point', 'Journey Point')))
...
def location_create_handler(request):
    ...
    if form.is_valid():
        ...
        LocationPoint.objects.get_or_create(
            user=request.user, location_name=location_name, location_type=location_type)
    else:
        error = "The form is not valid. Some of the required fields not entered."
...
class JourneyCreationForm1(forms.Form):
    ...
    """
    travel_type = forms.ChoiceField(choices=(('Bus', 'Bus'), ('AC1 Train', 'AC1 Train'), (
        'AC2 Train', 'AC2 Train'), ("Cab", "Cab"), ("Sleeper Train", "Sleeper Train")), required=True)
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        jloc = LocationPoint.objects.filter(
            user=user, location_type="Journey Point")
        self.fields["location_1"] = forms.ChoiceField(
            choices=((x.location_name, x.location_name) for x in jloc))
        self.fields["location_2"] = forms.ChoiceField(
            choices=((x.location_name, x.location_name) for x in jloc))
        self.layout = Layout(Fieldset("Provide the journey checkpoints here", Row(
            "location_1", "travel_type", "location_2")))
...
class JourneyCreationForm2(forms.Form):
    ...
    layout = Layout(Fieldset("Provide the journey information here",
                             "journey_name", Row("travel_date", "cotravel_number")))
...
class JourneyModifyForm(forms.Form):
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        self.fields["journey_name"] = forms.ChoiceField(
            choices=((x, x.journey_id) for x in jour))
        self.layout = Layout(Fieldset(
            "Provide the journey here and select one of the options", "journey_name"))
...
def user_journeys(request):
    ...
    for o, dis in matching_jlist:
        ...
        d["journey_checkpoints"] = [
            x.location.location_name for x in JourneyPoint.objects.filter(journey=o)]
        d["disable"] = dis
        lis.append(d)
    # journey add request list
    req_lis = Notification.objects.filter(
        user_to=request.user, notif_type="Journey Related", resolved="No")
...
def journey_modify_handler(request):
    ...
    global matching_jlist
    pprint(request.POST)
    jrny = Journey.objects.get(
        participants__in=[request.user], journey_id=request.POST.get("journey_name"))
...
def request_resolve_handler(request):
    ...
    if(typ == 0):
        ...
        print(jrny_id)
        jrny = Journey.objects.get(
            participants__in=[request.user], journey_id=jrny_id)
...
class TripCreationForm1(forms.Form):
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        tloc = LocationPoint.objects.filter(
            user=user, location_type="Trip Point")
        self.fields["location"] = forms.ChoiceField(
            choices=((x.location_name, x.location_name) for x in tloc))
        # self.fields["location_2"] = forms.ChoiceField(choices=((x.location_name,x.location_name) for x in jloc))
        self.layout = Layout(
            Fieldset("Provide the trip locations here", Row("location")))
...
class TripCreationForm2(forms.Form):
    ...
    layout = Layout(Fieldset("Provide the trip details here", "trip_name", "source", Row(
        "travel_date", "cotravel_number"), Row("duration", "expected_budget"), "trip_info"))
...
class TripModifyForm(forms.Form):
    def __init__(self, user, *args, **kwargs):
        ...
        self.fields["trip_name"] = forms.ChoiceField(
            choices=((x.trip_id, x.trip_id) for x in trips))
        self.layout = Layout(
            Fieldset("Provide the trip here and select one of the options", "trip_name"))
...
def trip_modify_handler(request):
    ...
    global matching_tlist
    pprint(request.POST)
    trp = Trip.objects.get(
        participants__in=[request.user], trip_id=request.POST.get("trip_name"))
    if("search" in request.POST):
